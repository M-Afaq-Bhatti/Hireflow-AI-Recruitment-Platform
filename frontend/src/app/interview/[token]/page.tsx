...
export default function InterviewRoomPage() {
  ...
  if (state === 'loading') return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center">
      <div className="text-gray-400">Verifying interview link…</div>
    </div>
  );
  ...
  if (state === 'error') return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
      <div className="text-center">
        ...
        <h2 className="text-xl font-bold text-white mb-2">Interview Unavailable</h2>
  ...
  if (state === 'completed') return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center px-4">
      <div className="max-w-lg text-center space-y-6">
        ...
        <p className="text-gray-400">Thank you {data?.candidateName}. Your responses have been recorded and will be reviewed.</p>
        <div className="card text-left space-y-3">
          <p className="text-sm font-semibold text-gray-300">Your Answers Summary</p>
          {data?.questions?.map((q: any, i: number) => (
            <div key={i} className="border-b border-gray-800 pb-3 last:border-0">
              <p className="text-xs text-gray-500 mb-1">Q{i+1}: {q.question}</p>
              <p className="text-sm text-gray-300 italic">"{allAnswers[i] || 'No response'}"</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="w-full max-w-xl space-y-6">
        ...
        <div className="text-center">
          <div className="inline-flex items-center gap-2 bg-purple-500/10 border border-purple-500/20 rounded-full px-4 py-1.5 mb-4">
            <div className={`w-2 h-2 rounded-full ${state === 'live' || state === 'question' || state === 'listening' ? 'bg-red-400 animate-pulse' : 'bg-gray-500'}`} />
            <span className="text-purple-300 text-sm">AI Interview Room</span>
          </div>
          <h1 className="text-xl font-bold text-white">{data?.jobTitle}</h1>
          <p className="text-gray-400 text-sm mt-1">{data?.candidateName}</p>
        </div>

        {/* AI Avatar */}
        <div className="card text-center py-8">
          <div className={`w-24 h-24 rounded-full mx-auto mb-4 flex items-center justify-center text-4xl transition-all duration-300 ${speaking ? 'bg-indigo-600 ring-4 ring-indigo-400 ring-opacity-50 scale-110' : 'bg-gray-800'}`}>
            🤖
          </div>
          <div className="text-sm text-gray-400">
            ...
            {speaking && <span className="text-indigo-400 font-medium"> — Speaking…</span>}
          </div>
        ...
        {state === 'listening' && (
          <div className="card">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="ml-auto text-xs text-gray-500">{timeLeft}s left</span>
            </div>
            <p className="text-sm text-gray-300 min-h-16 italic">
        ...
        {(state === 'question' || state === 'listening') && data?.questions?.[currentQ] && (
          <div className="bg-indigo-900/20 border border-indigo-700/40 rounded-xl px-5 py-4">
            <div className="text-xs text-indigo-400 font-medium mb-1">Current Question</div>
            <p className="text-gray-200">{data.questions[currentQ].question}</p>
          </div>
        )}
        ...
        {state === 'ready' && (
          <div className="space-y-3">
            <div className="bg-gray-800/50 rounded-lg p-4 text-sm text-gray-400 space-y-2">
              <p>📋 <strong className="text-gray-300">{data?.questions?.length} questions</strong> — about 10 minutes total</p>
              <p>🎤 Make sure your <strong className="text-gray-300">microphone is allowed</strong> in the browser</p>
              <p>🔇 Find a <strong className="text-gray-300">quiet place</strong> — the AI listens for 60 seconds per question</p>
            </div>
